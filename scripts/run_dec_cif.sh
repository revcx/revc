...
for QP in 22 27 32 37
do
md5sum -b                     ./tmp/test_cif_ld_i_q${QP}_etm_dec.yuv          | awk '{print $1,"./tmp/test_cif_ld_i_q'${QP}'_revc_dec.yuv"}'           > ./tmp/test_cif_ld_i_q${QP}_revc_yuv.txt
md5sum -b                     ./tmp/test_cif_ld_p_q${QP}_etm_dec.yuv          | awk '{print $1,"./tmp/test_cif_ld_p_q'${QP}'_revc_dec.yuv"}'           > ./tmp/test_cif_ld_p_q${QP}_revc_yuv.txt
md5sum -b                     ./tmp/test_cif_ld_b_q${QP}_etm_dec.yuv          | awk '{print $1,"./tmp/test_cif_ld_b_q'${QP}'_revc_dec.yuv"}'           > ./tmp/test_cif_ld_b_q${QP}_revc_yuv.txt

for BFRM in 1 3 7 15
do
md5sum -b                     ./tmp/test_cif_ra_b${BFRM}_q${QP}_etm_dec.yuv   | awk '{print $1,"./tmp/test_cif_ra_b'${BFRM}'_q'${QP}'_revc_dec.yuv"}'  > ./tmp/test_cif_ra_b${BFRM}_q${QP}_revc_yuv.txt
done
done
